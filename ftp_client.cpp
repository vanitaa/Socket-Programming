...
void show_menu(int file_descriptor){	
	...
	while(loop_invariant == 0){
		cout<<"Enter command to"<<endl<<"1. get list of files"<<endl<<"2. download file"
		 <<endl<<"3. upload file" <<endl<<"4. get current time "<<endl<<"5. get current directory"<<endl<<"6. create new file "<<endl
		 <<"7. delete file"<<endl<<"8. exit"<<endl;
		cin>>choice;
		switch(choice){
			...
			case 3:cout<<"enter the file name to upload"<<endl;
					...
					break;
			case 4: cout<<"Current time at server is: "<<endl;
					send(file_descriptor,string("time"),256);
					cout<<recieve(file_descriptor,25);
					break;
			case 5: cout<<"Current working directory is: "<<endl;
					send(file_descriptor,string("whereami"),256);
					cout<<recieve(file_descriptor,25);
					break;
			case 6: cout<<"Enter file name to create"<<endl;
					cin>>file_name;
					send(file_descriptor,string("touch")+" "+file_name,256);
					break;
			case 7: cout<<"Enter file name to delete"<<endl;
					cin>>file_name;
...
int main(){
	...
	if(file_descriptor < 0){
		cout<<"Error, failed to create sockets"<<endl;
   			...
   			server_socket.sin_addr.s_addr = inet_addr("127.0.0.1"); //IP can be changed over here.
